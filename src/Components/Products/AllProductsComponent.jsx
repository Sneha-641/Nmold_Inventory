...
const AllProductsComponent = ({searchQuery}) => {
    ...
    React.useEffect(() => {
        if (!searchQuery) {
            setState(allproductdata); // Reset to original data if searchQuery is empty
            return;
        }
    
        const searchData = allproductdata.filter(item => 
                ...
                <div className=''>
                    ...
                    <table className='w-full dash-table1 bg-white'>
                        <tr className='border border-solid cursor-pointer border-black h-[2rem]' style={{ backgroundColor: "white" }}>
                            ...
                            <th onClick={() => sortdata('name2')}> Product Name</th>
                            <th onClick={() => sortdata('name3')}> Unit</th>
                            <th onClick={() => sortdata('name4')}> Category</th>
                            <th onClick={() => sortdata('name5')}> Supplier</th>
                            <th onClick={() => sortdata('name5')}> Amount</th>
                            <th onClick={() => sortdata('name6')}> Stock</th>
                            <th onClick={() => sortdata('name7')}> Action</th>
                        </tr>
                        {state.slice(start, start + interval).map((item, index) => (
                            <tr className='h-[2rem]'>
                                ...
                                <td><div className='flex text-center justify-evenly'>
                                        <div className='py-[2px] cursor-pointer' onClick={()=>handlePopup('update',21)}>
